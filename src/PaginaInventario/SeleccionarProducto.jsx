...
function SeleccionarProducto ({listaItems, seleccionar, desactivar}) {
  ...
  // Actualizar la lista seleccionable
  useEffect(() => {
    if(listaItems.length){
      setValue('NOMBRE', listaItems[0].NOMBRE)
    }
  }, [listaItems])      
  ...
  return(
    <Formulario
      ...
      <InputProducto
          type = 'text'
          list = 'Opciones-producto'
          disabled = {desactivar}
          {...register('NOMBRE')} />
        <datalist id = 'Opciones-producto'>
            {listaItems.map( (item) => (
              <option
                key = {item.ID} 
      ...
      <Seleccionar
        disabled = {listaItems.length === 0 || desactivar}
        value = 'Seleccionar'
        type = 'submit' />
    </Formulario>
  )
